...
방향 센서는 현재 장치의 자세를 나타낸다.
3개의 값이 제공되는데 각각 방위각(azimuth), 피치(pitch), 롤(roll)이라고 불린다.
이 3개의 값은 물체가 3차원 공간에서 x, y, z축을 기준으로 얼마나 회전했는지를 나타낸다.

- 방위각(azimuth)
방위각은 장치가 북쪽을 향하고 있는지 아니면 남쪽을 향하고 있는지를 나타낸다.
간단히 말하면 나침판이 가리키는 방향을 각도로 표시한 것이다.
0도는 북쪽이라는 의미이고 90도는 동쪽, 180도는 남쪽, 270도는 서쪽을 나타낸다.

- 피치(pitch)
y축을 기준으로 장치가 얼마나 회전하였는지를 나타낸다. 장치가 평평한 곳에 놓여져 0도이고, 수직에 서있을 때는 -90도, 거꾸로 서있을 때는 90도가 된다.

- 롤(roll)
장치가 z축을 기준으로 얼마나 회전하였는지를 나타낸다.
용어 그대로 장치가 돌아가는 것을 생각하면 된다.
장치가 바닥에 똑바로 놓여져 있을 때는 0도이고, 화면이 오른쪽에 있으면 90도이고 화면이 왼쪽에 있으면 -90도이다.

### 3. 가속도 센서

가속도 센서(accelerometer)는 장치의 가속도를 측정한다.
가속도는 속도의 변화율이다.
따라서 속도와는 약간 다르다.
속도를 미분하면 가속도가 된다.
반대로 가속도를 적분하면 속도가 된다.
가속도 센서는 중력 센서(gravity sensor)라고도 한다.

아마 많이 들어보았을 지포스(g-force)라는 용어가 바로 가속도를 나타낸다.
지포스는 중력 가속도를 1로 하여서 측정한 가속도를 말한다.
보통 전투기 조종사가 받는 최대 지포스가 9G라고 한다.
즉 중력 가속도의 9배 정도라고 생각할 수 있다.

가속도도 3차원 공간에서 측정되기 때문에 3개의 값을 가진다.
즉 x축 방향의 가속도, y축 방향의 가속도, z축 방향의 가속도가 측정되고 보고된다.

## **17. NFC**

### 1. NFC

NFC(Near Field Communication)은 단거리 무선 통신 기술의 일종이다.
보통 1~4cm 이하의 거리에서 통신이 이루어진다.
NFC는 NFC 태그와 안드로이드 장치 사이에서 적은 양의 데이터를 공유할 수 있게 한다.
NFC통신에는 시작자(initiator)와 타깃(target)이 있다.
시작자는 능동적으로 RF필드를 발생한다.
이 RF필드는 타깃에 전원을 공급할 수 있다.
따라서 타깃은 전원장치가 없어도 되므로 아주 간단하게 제작할 수 있다.
예를 들어서 태그(tags), 스티커(stickers), 카드(cards)의 형태로 제작이 가능하다.
물론 타겟이 전원을 가지고 있는 경우에도 통신은 가능하다.

NFC의 가장 큰 특징은 연결 절차가 아주 간단하다는 것이다.
블루투스처럼 발견이나 페어링과 같은 절차가 필요 없다.
또 패스워드를 입력할 필요가 없다.
NFC를 사용하려면 스마트폰을 아주 가까이 접촉하여야 되므로 보안이나 사생활 침해의 염려가 비교적 적다.
그리고 스마트폰의 화면이 꺼져있는 경우에는 NFC가 동작하지 않는다.
따라서 이러한 특징들을 이용하여서 많은 흥미로운 애플리케이션을 제작할 수 있다.

NFC 태그는 복잡도에 따라 상당한 차이가 있다.
제일 간단한 태그는 읽기와 쓰기만을 제공한다.
더 복잡한 태그는 산술 연산을 제공할 수 있으며 암호화 하드웨어를 가지고 있다.
가장 복잡한 태그는 운영체제를 가지고 있어서 태그 상에서 코드를 실행할 수 있다.
태그에 저장된 데이터는 여러 가지의 형식으로 저장될 수 있으나 안드로이드에서는 NFC 포럼의 표준 규격인 NDEF(NFC Data Exchange Format)를 사용한다.

```
다른 기술과 NFC의 비교

블루투스나 와이파이 같은 무선 기술과 비교해보면, NFC는 매우 협소한 대역폭과 전송 거리를 가진다.
그러나 저비용으로 제작할 수 있으며 전원이 없는 타깃도 가능하고 복구나 페어링을 요구하지 않는 것은 장점이라 할 수 있다.
상호작용은 장치를 대기만 해도 시작된다.
NFC 하드웨어를 갖춘 안드로이드 장치는 스크린에 켜져 있는 동안에는 시작자로 동작하며 NFC 태그가 발견되면 적절한 앱을 실행한다.
```

#### NFC의 모드

NFC 기술은 주로 모바일 폰을 위한 것이다.
NFC 포럼에 따르면 현재 3가지의 모드가 있다.

```
- Reader/Writer 모드 : NFC 장치가 활성화되어 있고 주로 수동적인 태그를 읽는다. 예를 들면 스마트 포스터를 구현할 수 있다.
...
가장 많이 사용될 것으로 예상되는 분야는 모바일 지불이다.
철도, 지하철, 항공, 영화관, 콘서트, 스포츠 행사 시 모바일 티켓으로 사용할 수 있다.
사용자는 NFC 기술이 장착된 스마트폰을 사용하여서 직불이나 신용카드처럼 지불할 수 있다.

모바일 태그로도 사용할 수 있다.
스마트폰을 태그에 접촉하면 태그에 저장된 정보를 읽을 수 있다.
거리의 간판이나 게시판에 붙여진 포스터의 태그를 읽어서 정보나 광고를 NFC 장치를 통하여 얻을 수 있다.
예를 들어서 포스터에 내장된 태그에 접촉하면 웹사이트로 자동으로 연결될 수 있다.
QR코드와 비슷하지만 사용하기가 아주 편리하다.

또 하나의 유망한 분야는 2개의 스마트폰을 접촉하여서 데이터를 전송하는 분야이다.
예를 들어서 페이스북에 저장된 친구들의 정보를 접촉을 통하여 다른 스마트폰으로 전송할 수 있다.
또 NFC를 이용한 게임도 가능할 것이다.

### 2. NFC의 기초

먼저 NDEF라고 불리는 NFC 안의 데이터의 구조를 살펴본다.
또 NFC 태그에 NDEF를 저장하는 방법과 NFC 태그에서 NDEF를 읽는 방법을 알아본다.
그리고 NFC 태그에 스마트폰을 접촉하면 어떤 작업들이 진행되는지를 학습한다.

전체 NDEF 스펙을 다운로드하려면 NFC 포럼의 Specification 다운로드 사이트로 가야한다(http://www.nfc-forum.org/specs/spec_license).

#### NFC에서 사용되는 데이터의 형식

NFC에서 사용되는 데이터의 형식은 NDEF(NFC Data Exchange Format)이라고 불린다.
NDEF 메시지가 태그와 스마트폰 사이에서 전송된다.

NDEF 데이터의 구조를 간단히 살펴보자.
NDEF 메시지는 여러 개의 NDEF 레코드로 이루어질 수 있다.
NDEF 레코드는 크게 헤더와 페이로드로 나누어진다.
헤더는 페이로드의 타입을 지정하고 실제 데이터는 페이로드로 저장된다.
NDEF 메시지의 목적은 특정 타입의 페이로드를 전송하는 것이다.
가장 많이 사용되는 페이로드는 "http://..."와 같은 URI나 또는 텍스트 형태이다.

각각의 NDEF 레코드는 다음과 같은 필드들을 가지고 있다.

- 3-bit TNF(Type Name Format) - 가변 길이 필드들을 어떻게 해석할 것인지를 나타낸다.
유효한 값 중에서 많이 사용되는 값은 다음과 같다.

```
TNF_ABSOLUTE_URI : 페이로드에는 URI가 저장된다(UTF-8로 인코딩된다).
TNF_MIME_MEDIA : 페이로드에는 타입 필드에서 지정된 MIME 타입의 데이터가 저장되어 있다.
TNF_WELL_KNOWN : 잘 알려진 값들로 레코드 타입 정의(Record Type Definition)에 따라서 MIME 타입이나 URI를 결정한다.
```

- 타입(type) - 레코드의 타입을 나타낸다.
만약 TNF의 값이 TNF_WELL_KNOWN이라면 이 필드는 Record Type Definition(RTD)이 된다.
많이 사용되는 RTD 값은 다음과 같다.
...
- 아이디(ID) - 레코드의 유일한 식별자.
이 필드는 자주 사용되지는 않지만 개발자는 아이디를 부여할 수 있다.

- 페이로드(payload) - 여기에 실제 데이터가 저장된다.
NDEF 메시지는 여러 개의 레코드를 가질 수 있으므로 첫 번째 레코드에 전체 데이터가 들어있지 않을 수도 있다.

#### NDEF 메시지의 예

예를 들어서 다음의 가장 많이 사용되는 NFC 레코드들의 형식을 보여준다.
NDEF 레코드를 나타내는 사각형에서 첫 번째 줄이 TNF이고 두 번째 줄이 타입 필드, 세 번째 줄이 페이로드이다.

- 첫 번째 레코드의 경우, TNF는 "TNF_WELL_KNOWN"이고 타입은 RTD_TEXT이다.
따라서 텍스트 타입이고 페이로드에는 "The lazy dog jumped..." 이다.

- 두 번째 레코드의 경우, MIME 데이터이고 타입은 text/plain이며 페이로드는 역시 텍스트 데이터 "The lazy dog jumped..."이다.
이들 레코드에 대해서 안드로이드는 인텐트의 MIME 타입 필드에 text/plain을 저장한 후에, 이 인텐트를 가지고 적절한 애플리케이션을 시작한다.
애플리케이션은 자신이 관심있는 데이터 타입에 대하여 인텐트 필터를 가지고 있다.

- 세 번째 레코드의 경우, 제조사 MIME 타입으로서 만약 개발자가 스티커와 같은태그에 넣으려고 미리 생각해둔 자료 형식이 있다면 이 형식을 사용하면 된다.
또 peer-to-peer 통신을 원하는 경우에는 이 형식을 사용한다.
"application/vnd."에 회사의 도메인 이름을 붙이면 된다.
페이로드에는 개발자가 생각한 데이터를 저장하면 된다.
물론 이러한 자료 형식을 처리하는 애플리케이션을 작성하고 매니페스트 파일의 android:mimeType 속성에 동일한 "application/vnd..." 을 지정하면 NFC 태그가 인식되었을 경우에 바로 실행된다.

- 네 번째 레코드의 경우, TNF는 "TNF_WELL_KNOWN"이고 타입은 RTD_URI이다.
URI는 NFC를 통하여 전달하는 예이다.

### 3. 안드로이드는 NFC를 어떻게 처리하는가?

NFC 태그를 안드로이드 장치가 처리하려면 먼저 장치에서 NFC가 사용 가능하도록 설정되어 있어야 한다.
"환경 설정"에서 NFC 기능을 켤 수 있다.
[설정] > [연결 및 파일 주고받기]로 들어가서 NFC를 활성화하여야 한다.

#### 매니페스트 파일에 NFC 접근 권한 요청하기

NFC 애플리케이션을 작성할 때는 다음과 같은 권한을 매니페스트 파일을 통하여 사용자에게 요청하여야 한다.

- NFC 하드웨어를 사용하고 NFC 인텐트를 처리하는 권한을 요청한다.

`<uses-permission android:name="android.permission.NFC" />`

- NFC를 처리하려면 안드로이드 버전이 10 이상이어야 한다.

`<uses-sdk android:minSdkVersion="10" />`

- uses-feature 요소를 사용하여서 구글 플레이에서 NFC 하드웨어를 가진 장치에서만 개발자의 앱이 나타나게 하여야 한다.

`<uses-feature android:name="android.hardware.nfc" android:required="true" />`

#### NFC 태그가 애플리케이션으로 전달되는 방법

안드로이드 장치에 NFC 태그를 접촉하였다고 가정하자.
어떤 일들이 발생할까?
안드로이드 장치는 화면이 켜져 있는 동안은 항상 NFC 태그를 검색한다.
안드로이드 장치가 NFC 태그를 찾으면 가장 적절한 액티비티가 자동으로 이 태그를 처리하게 하는 것이 바람직하다.
이때에 사용자에게 확인하는 과정은 생략된다.
왜냐하면 NFC 태그는 아주 가까운 거리에서 스캔되기 때문에 사용자가 액티비티를 선택하기 위해서 장치를 움직이면 태그에서 멀어지게 되고 연결이 끊기게 된다.

원칙적으로 NDEF 데이터는 하나의 액티비티만을 구동한다.
첫 번째 NDEF 레코드의 타입이 어떤 액티비티가 구동되는지를 결정한다.
안드로이드가 스캔된 NFC 태그를 분석하여, NFC 태그에 관심이 있는 애플리케이션을 찾는 시스템을 태그 구동 시스템이라고 한다.
태그 구동 시스템은 다음과 같은 순서로 동작한다.

1. 안드로이드 장치가 NFC 태그를 스캔하게 되면, NDEF 메시지를 파싱하여 TNF와 타입 필드를 이용하여서 NDEF 메시지의 MIME 타입이나 URI를 찾는다.
